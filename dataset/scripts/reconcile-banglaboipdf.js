...
const requestedRunId = process.env.BANGLABOIPDF_RUN_ID || null;
const checkId = process.env.BANGLABOIPDF_RECONCILE_ID || new Date().toISOString().replace(/[:.]/g, "-");
const limit = Number(process.env.BANGLABOIPDF_RECONCILE_LIMIT || 0);
...
const titleStopWords = new Set(["a", "an", "and", "bangla", "bengali", "book", "boi", "by", "ebook", "pdf", "the", "o"]);
...
function looseTitleKey(value) {
  return key(value)
    ?.replace(/\b(?:ed|edition|vol|volume|part|khanda|khondo|book|boi|ebook|pdf)\b/g, " ")
...
function selectedRun() {
  ...
  const dir = path.join(archiveDir, "banglaboipdf", requestedRunId);
...
function banglaParentheticals(value) {
  const text = cleanText(value);
  if (!text) return [];
  const matches = [];
  for (const match of text.matchAll(/[([]([^()[\]]*[\u0980-\u09FF][^()[\]]*)[)\]]/g)) {
    matches.push(match[1]);
  }
  return matches;
}

function withoutParentheticals(value) {
  return cleanText(String(value || "").replace(/[([][^()[\]]*[\u0980-\u09FF][^()[\]]*[)\]]/g, " "));
}

function recordTitleValues(record) {
  return unique([
    ...banglaParentheticals(record.title),
    cleanTitle(withoutParentheticals(record.title)),
    cleanTitle(record.title),
    ...banglaParentheticals(record.raw_post_title),
    cleanTitle(record.raw_post_title),
    cleanTitle(record.slug?.replace(/-/g, " "))
  ]);
}
...
function recordContributorLabels(record) {
  const labels = [];
  if (!isGenericAuthor(record.author)) labels.push(record.author);
  if (!isGenericAuthor(record.editor_or_translator)) labels.push(record.editor_or_translator);
...
function classifyRecord(record, index) {
  ...
  if (exactTitleOnly.length) {
    return {
      ...
      reason: contributorLabels.length ? null : "BanglaBoiPDF has no usable contributor label",
  ...
  if (best && ((best.title_score >= 0.82 && best.author_score >= 0.5) || best.title_score >= 0.94)) {
    return {
      ...
      reason: contributorLabels.length ? null : "title-based only; no usable BanglaBoiPDF contributor",
...
function reportRow(record, classification) {
  ...
  return {
    ...
    raw_title: record.raw_post_title,
    source_title: record.title,
    authors: contributors,
    source_author: cleanText(record.author),
    editor_or_translator: cleanText(record.editor_or_translator),
    url: record.url,
    ...
    flags: {
      ...
      generic_author_seen: isGenericAuthor(record.author),
