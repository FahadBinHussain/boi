...
const requestedRunId = process.env.SMALL_ISLAMIC_RUN_ID || null;
const researchFileName =
  process.env.SMALL_ISLAMIC_RESEARCH_FILE || "candidate_small_islamic_sources_2026_06_01.json";
...
const genericAuthorPattern =
  /^(?:book|books?|ebook|ebooks?|pdf|author|authors?|unknown|anonymous|various|various writers?|various author|collection|compiled|islamic|islamic books?|www\..+|https?:\/\/.+)$/i;
...
function selectedRun() {
  ...
  if (!fs.existsSync(path.join(dir, "manifest.json"))) {
    throw new Error(`Small Islamic run has no manifest: ${dir}`);
  }
...
function readSourceRows(run) {
  const rows = [];
  for (const slug of ["islamhouse", "yshamsan", "hadithone"]) {
...
function usableContributors(row) {
  return (row.contributors || []).filter((contributor) => ["author", "translator", "editor"].includes(contributor.role));
}
...
function sourceLabel(row) {
  if (row.source === "IslamHouse") return "IslamHouse Bengali books catalog";
  if (row.source === "YShamsan") return "YShamsan Bengali books catalog";
  if (row.source === "Hadith.one") return "Hadith.one Bengali book page";
  return `${row.source} book page`;
}

function recordType(row) {
  if (row.format) return `${String(row.format).toLowerCase()} book`;
  return "book";
}
...
function groupSources(sourceRows) {
  ...
  for (const group of groups.values()) {
    ...
    group.contributors = group.contributors.filter((contributor) => {
      const identity = `${key(contributor.name)}|${contributor.role}`;
      ...
      group.rows
        .map((row) =>
          usableContributors(row)
            .map((contributor) => `${key(contributor.name)}|${contributor.role}`)
            .sort()
            .join(";")
        )
        .filter(Boolean)
  ...
  return group.rows.map((row) => ({
    ...
    notes:
      match.reason === "exact_title_and_contributor"
        ? "Current catalog page confirms the same normalized title and at least one matching contributor."
        : "Current catalog page confirms the exact title and supplies usable contributor metadata where the candidate had only generic or missing author text."
  }));
}
...
function buildResearchItem(candidate, group, match) {
  ...
  const format = unique(group.rows.map((row) => row.format || recordType(row))).join(", ") || "book";
