...
const requestedRunId = process.env.STRONG_RETAILER_RUN_ID || null;
const researchFileName = process.env.STRONG_RETAILER_RESEARCH_FILE || "candidate_strong_retailer_sources_2026_06_02.json";
const sourceDirs = (process.env.STRONG_RETAILER_SOURCES || "pbs,prothoma,kitabghor,bdbooks")
  .split(",")
...
function selectedRun() {
  ...
  if (!fs.existsSync(path.join(dir, "manifest.json"))) {
    throw new Error(`Strong retailer source run has no manifest: ${dir}`);
  }
...
function sourceLabel(row) {
  if (row.source === "PBS") return "PBS book catalog page";
  if (row.source === "Prothoma") return "Prothoma product detail page";
  if (row.source === "Kitabghor") return "Kitabghor product detail page";
  if (row.source === "BDBOOKS") return "BDBOOKS product detail page";
  return `${row.source} book page`;
}

function recordType(row) {
  if (row.source === "PBS") return "book listing";
  return "book detail";
}
...
function main() {
  ...
  writeJson(researchPath, {
    ...
    research_scope:
      "Automated exact-title reconciliation for pending generated BOI candidates against the metadata-only PBS, Prothoma, Kitabghor, and BDBOOKS scrape. Promotes only exact normalized-title matches with matching contributor evidence, or exact-title rows where the candidate had only generic/missing author metadata and the current catalog supplies usable contributors.",
