...
const requestedRunId = process.env.BANGLABOOKS_IN_RUN_ID || null;
const checkId = process.env.BANGLABOOKS_IN_RECONCILE_ID || new Date().toISOString().replace(/[:.]/g, "-");
const limit = Number(process.env.BANGLABOOKS_IN_RECONCILE_LIMIT || 0);
...
function recordTitleValues(record) {
  return unique([record.title_bn, cleanTitle(record.title), cleanTitle(record.raw_post_title), cleanTitle(record.slug?.replace(/-/g, " "))]);
}
...
function recordContributorLabels(record) {
  if (isGenericAuthor(record.author)) return [];
  return unique([record.author]);
}
...
function classifyRecord(record, index) {
  ...
  if (exactTitleOnly.length) {
    return {
      ...
      reason: contributorLabels.length ? null : "BanglaBooks.in has no usable contributor label",
  ...
  if (best && ((best.title_score >= 0.82 && best.author_score >= 0.5) || best.title_score >= 0.94)) {
    return {
      ...
      reason: contributorLabels.length ? null : "title-based only; no usable BanglaBooks.in contributor",
