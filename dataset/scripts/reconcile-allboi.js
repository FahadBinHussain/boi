...
const requestedRunId = process.env.ALLBOI_RUN_ID || null;
const checkId = process.env.ALLBOI_RECONCILE_ID || new Date().toISOString().replace(/[:.]/g, "-");
const limit = Number(process.env.ALLBOI_RECONCILE_LIMIT || 0);
...
const genericAuthorPattern = /\b(?:allboi|author|authors|various|various writers?|various author|unknown|compiled|collection)\b/i;
...
function recordTitleValues(record) {
  return unique([record.title_bn, cleanTitle(record.title), cleanTitle(record.raw_page_title), cleanTitle(record.slug?.replace(/-/g, " "))]);
}
...
function classifyRecord(record, index) {
  ...
  if (exactTitleOnly.length) {
    return {
      ...
      reason: contributorLabels.length ? null : "Allboi has no usable contributor label",
  ...
  if (best && ((best.title_score >= 0.82 && best.author_score >= 0.5) || best.title_score >= 0.94)) {
    return {
      ...
      reason: contributorLabels.length ? null : "title-based only; no usable Allboi contributor",
...
function reportRow(record, classification) {
  ...
  return {
    ...
    download_count: record.download_count || null,
