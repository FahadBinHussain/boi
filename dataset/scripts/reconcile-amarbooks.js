...
const requestedRunId = process.env.AMARBOOKS_RUN_ID || null;
const checkId = process.env.AMARBOOKS_RECONCILE_ID || new Date().toISOString().replace(/[:.]/g, "-");
const limit = Number(process.env.AMARBOOKS_RECONCILE_LIMIT || 0);
...
const titleStopWords = new Set(["a", "an", "and", "book", "by", "ebook", "ebooks", "pdf", "the", "o"]);
...
function looseTitleKey(value) {
  return key(value)
    ?.replace(/\b(?:ed|edition|vol|volume|part|khanda|khondo|book|rana|series)\b/g, " ")
...
function latestAmarBooksRunDir() {
  const root = path.join(archiveDir, "amarbooks");
  if (!fs.existsSync(root)) throw new Error(`No AmarBooks archive directory found: ${root}`);
  const runs = fs
  ...
  if (!runs.length) throw new Error(`No AmarBooks books.jsonl run found in ${root}`);
  return runs[0];
}

function selectedRun() {
  if (!requestedRunId) return latestAmarBooksRunDir();
  const dir = path.join(archiveDir, "amarbooks", requestedRunId);
  const filePath = path.join(dir, "books.jsonl");
  if (!fs.existsSync(filePath)) throw new Error(`AmarBooks run has no books.jsonl: ${filePath}`);
...
function recordTitleValues(record) {
  return unique([record.title_bn, record.title, record.schema_headline, record.raw_page_title].map((value) => {
...
function recordAuthorLikeLabels(record, index) {
  const label = cleanText(record.category_name);
  if (!label) return [];
  const labelKey = key(label);
  if (index.authorIdsByKey.has(labelKey)) return [label];
  return [];
}
...
function bestAuthorScore(recordAuthorLabels, work) {
  if (!recordAuthorLabels.length || !work.author_keys.length) return 0;
  let best = 0;
  for (const label of recordAuthorLabels) {
...
function candidateWorkIds(record, index) {
  ...
  const labelKey = key(record.category_name);
  for (const id of index.authorKeyToWorkIds.get(labelKey) || []) ids.add(id);
...
function classifyRecord(record, index) {
  ...
  const recordAuthorLabels = recordAuthorLikeLabels(record, index);
  const exactTitleIds = unique(titleValues.flatMap((title) => index.titleKeyToWorkIds.get(key(title)) || []));
  const exactAuthorIds = new Set(recordAuthorLabels.flatMap((label) => index.authorIdsByKey.get(key(label)) || []));
  ...
  for (const work of candidates) {
    ...
    const authorScore = bestAuthorScore(recordAuthorLabels, work);
    const totalScore = Math.round((titleScore * 0.78 + authorScore * 0.22) * 1000) / 1000;
    const current = {
      work,
  ...
  if (exactTitleOnly.length) {
    return {
      ...
      reason: recordAuthorLabels.length ? null : "AmarBooks label is not confirmed as a main author",
      matches: exactTitleOnly.map((work) =>
        compactWork(work, {
          title_score: 1,
          author_score: bestAuthorScore(recordAuthorLabels, work)
        })
      )
    };
  }

  if (best && best.title_score >= 0.9 && best.author_score >= 0.78) {
    return {
  ...
  if (best && ((best.title_score >= 0.82 && best.author_score >= 0.5) || best.title_score >= 0.94)) {
    return {
      ...
      reason: recordAuthorLabels.length ? null : "title-based only; AmarBooks label is not a confirmed author",
  ...
  return {
    ...
    reason: recordAuthorLabels.length ? null : "AmarBooks label is category/series or unconfirmed author",
    matches: hasReviewableBestMatch(best) ? [compactBestMatch(best)] : []
  };
}

function reportRow(record, classification, index) {
  return {
    ...
    category_author_confirmed: recordAuthorLikeLabels(record, index).length > 0,
    url: record.url,
...
function main() {
  ...
  for (const record of records) {
    ...
    const row = reportRow(record, classification, index);
  ...
  const manifest = {
    ...
    interpretation: {
      exact_matches: "same title and AmarBooks category label matches a main author; safe existing-main candidate for possible source-link enrichment",
      title_only_matches: "same title exists but AmarBooks label is not confirmed as the main author",
      likely_duplicates: "probable duplicate by title and confirmed author label; do not promote as new",
