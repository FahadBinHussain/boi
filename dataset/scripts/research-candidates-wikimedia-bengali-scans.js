...
const requestedRunId = process.env.WIKIMEDIA_BENGALI_RUN_ID || null;
const researchFileName =
  process.env.WIKIMEDIA_BENGALI_RESEARCH_FILE || "candidate_wikimedia_bengali_scans_2026_06_03.json";
...
const genericAuthorPattern =
  /^(?:book|books?|ebook|ebooks?|pdf|djvu|scan|scans?|file|author|authors?|unknown|anonymous|various|various writers?|various author|collection|compiled|www\..+|https?:\/\/.+)$/i;
...
function selectedRun() {
  ...
  if (!fs.existsSync(path.join(dir, "manifest.json"))) {
    throw new Error(`Wikimedia Bengali scan run has no manifest: ${dir}`);
  }
...
function titleEntries(row) {
  return unique([row.title, row.title_en]).map((title) => ({ key: key(title), title }));
}
...
function matchCandidateToGroup(candidate, group) {
  ...
  for (const candidatePerson of usableCandidatePeople) {
    for (const row of group.rows) {
      const rowMatches = rowPeople(row).filter((sourcePerson) => peopleProbablyMatch(candidatePerson.name, sourcePerson.name));
      for (const sourcePerson of rowMatches) matchingPeople.push({ candidate: candidatePerson, source: sourcePerson });
      if (rowMatches.length && !matchingRows.includes(row)) matchingRows.push(row);
    }
  }
...
function sourceRowsForItem(group, match) {
  ...
  for (const row of group.rows) {
    rows.push({
      ...
      url: row.url,
      external_id: row.wikidata_item || row.source_id || null,
      record_type: "scan index",
      raw_title: row.title,
      ...
      notes:
        match.reason === "exact_title_and_contributor"
          ? `Bengali Wikisource Index metadata confirms the same normalized title and contributor for scan ${row.file_title || row.index_title}.`
          : `Bengali Wikisource Index metadata confirms the exact title and supplies usable contributor metadata where the candidate had only generic or missing author text.`
    });
    if (row.commons_page_url) {
      rows.push({
        source: "Wikimedia Commons",
        url: row.commons_page_url,
        external_id: row.commons_pageid || row.file_title || null,
        record_type: "scan file metadata",
        raw_title: row.file_title || row.title,
        raw_author: row.commons_artist || rawAuthor({ contributors: usableContributors(row), people: rowPeople(row) }),
        notes: `Commons file-page metadata for the scan linked from the Bengali Wikisource Index page${row.page_count ? `; page count ${row.page_count}` : ""}${row.license_short_name ? `; license ${row.license_short_name}` : ""}.`
      });
    }
  }
  return rows;
...
function buildResearchItem(candidate, group, match) {
  ...
  const pages = group.rows.map((row) => row.page_count).find(Boolean) || null;
  ...
  return {
    ...
    evidence:
      match.reason === "exact_title_and_contributor"
        ? `Bengali Wikisource lists ${group.title}${titleEn ? ` (${titleEn})` : ""}${contributorText ? ` with ${contributorText}` : ""}; the normalized title and contributor evidence match the pending candidate.`
        : `Bengali Wikisource lists ${group.title}${titleEn ? ` (${titleEn})` : ""}${contributorText ? ` with ${contributorText}` : ""}; the pending candidate has the same normalized title but only generic or missing author metadata, so this current scan index supplies the supporting source.`,
    evidence_sources: group.rows.flatMap((row) =>
      [
        {
          label: "Bengali Wikisource Index page",
          url: row.url,
          notes: `Wikisource scan index confirms title${usableContributors(row).length ? " and contributor metadata" : ""}${row.wikidata_item ? ` with Wikidata item ${row.wikidata_item}` : ""}.`
        },
        row.commons_page_url
          ? {
              label: "Wikimedia Commons file page",
              url: row.commons_page_url,
              notes: `Commons metadata for linked scan file${row.page_count ? ` with ${row.page_count} pages` : ""}.`
            }
          : null
      ].filter(Boolean)
    ),
...
function dedupeRows(rows) {
  ...
  return rows.filter((row) => {
    const identity = JSON.stringify([row.work_id, row.source_title, row.source_contributors, row.source_urls]);
...
function main() {
  ...
  const dedupedReviewRows = dedupeRows(reviewRows);

  writeJson(researchPath, {
    generated_at: generatedAt,
    research_scope:
      "Automated exact-title reconciliation for pending generated BOI candidates against the metadata-only Bengali Wikisource Index plus Commons scan metadata scrape. Promotes only exact normalized-title matches with matching contributor evidence, or exact-title rows where the candidate had only generic/missing author metadata and the current scan index supplies usable contributors.",
    ...
    selection_summary: {
      ...
      title_only_review: dedupedReviewRows.length
    },
    review_file: path.relative(archiveDir, reviewPath).replaceAll("\\", "/"),
    items
  });
  writeJsonl(reviewPath, dedupedReviewRows);
